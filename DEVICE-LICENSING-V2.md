...
4. The customer enters Store GUID (the exact database name), Store Name and ZIP, then selects **Copy Activation Request**.
5. Open the standalone developer-only Admin License Generator, connect to the licensing database, and select **Paste Activation Request**.
6. Paste the copied request. The generator verifies its device signature and finds the active subscription by Store GUID/database name.
